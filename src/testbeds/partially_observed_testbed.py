...
class PartiallyObservedPerfTestbed(Testbed):
    testbed_dir = "partially-observed-testbed"
    ...
    def __init__(self,
        ...
        super().__init__(task, perf_metric, perf_matrices, meta_feat, meta_features, n_splits, graph_names, all_models,
                         graph_domain, PartiallyObservedPerfTestbed.testbed_dir,
                         exclude_graphs_and_models_with_missing_performance=True)
        self.testbed_root = settings.TESTBED_ROOT / PartiallyObservedPerfTestbed.testbed_dir / task / \
                            f"{perf_metric}-sparsity-{perf_sparsity:.1f}"
        self.perf_sparsity = perf_sparsity
    ...
    def testbed_settings(self):
        return {
            ...
            'perf_sparsity': self.perf_sparsity,
            'exclude_graphs_and_models_with_missing_performance': self.exclude_graphs_and_models_with_missing_performance,
        }

    def sparse_perf_mat_path(self, mkdir=True):
        if mkdir and not self.testbed_root.exists():
            self.testbed_root.mkdir(parents=True, exist_ok=True)
        return self.testbed_root / f"P_{self.perf_metric}-sparsity-{self.perf_sparsity:.1f}.npy"

    def generate(self):
        P_partially_observed = self.create_partially_observed_perf_matrix(self.perf_mat, self.perf_sparsity)
        np.save(self.sparse_perf_mat_path(), P_partially_observed)
    ...
    @classmethod
    def create_partially_observed_perf_matrix(cls, perf_mat, perf_sparsity):
        """
        Return a partially observed performance matrix
        by replacing performance entries of P with NaNs, uniformly at random (per row)
        """
        perc_to_replace_with_nan = perf_sparsity
        assert 0 <= perc_to_replace_with_nan <= 1, perc_to_replace_with_nan
        P_partially_observed = perf_mat.copy()
        num_missing_per_row = int(np.ceil(P_partially_observed.shape[1] * perc_to_replace_with_nan))
        print(f"num_missing_per_row={num_missing_per_row} out of {P_partially_observed.shape[1]} (perf_sparsity={perf_sparsity})")

        rnd = np.random.RandomState(seed=1)
        for i in range(P_partially_observed.shape[0]):
            rand_perm = rnd.permutation(P_partially_observed.shape[1])
            rand_idx = rand_perm[0:num_missing_per_row]
            P_partially_observed[i, rand_idx] = np.nan

        return P_partially_observed

    def get_train_perf_mat(self):
        return np.load(self.sparse_perf_mat_path())
...
if __name__ == '__main__':
    ...
    for task in ['link-pred', 'node-class']:
        for perf_sparsity in [0.1, 0.3, 0.5, 0.7, 0.9]:
            PartiallyObservedPerfTestbed(
                task=task,
                perf_metric='map', perf_matrices=perf_matrices,
                meta_feat='regular', meta_features=meta_features,
                perf_sparsity=perf_sparsity, n_splits=5,
